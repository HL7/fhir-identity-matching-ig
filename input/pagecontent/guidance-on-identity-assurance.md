...
This section provides best practices to be used in addition to NIST 800-63-3 Digital Identity Guidelines for practical application in healthcare settings. These procedures can be used to achieve Identity Assurance Level 2 (IDIAL2) and other [identity assurance levels](glossary.html) between IDIAL1 and IDIAL2 in typical healthcare settings, considering the [identity evidence](glossary.html) generally available across patient populations and for others having a role in managing or exchanging health information and personally identifiable information.
...
NOTE: Although implementers, lacking more specific requirements that may exist in network participation agreements, likely want to perform their own risk analyses to determine the appropriate IDIAL for various use cases (for example, a patient’s access to their own data or a HIPAA Covered Entity's access to health data for Treatment, Payment, or Operations), this IG provides example [use cases](use-cases.html) in which certain input match invariants, match input data, and IDIALs are deemed appropriate based on industry feedback received. The guide also specifically cites recommended IDIALs for professional users and administrators in section 3.2. The IDIALs defined below are not currently specified in a code system or value set within this implementation guide (IG). Additionally, the intermediate identity assurance levels (IDIAL1.2-1.8) described below have NOT been endorsed by NIST in any capacity, though the team has requested their feedback. 
...
Social Security Number (on its own without presenting the card itself) does not have a role as evidence in IDIALs beyond IDIAL1.5 except as may be needed for identity resolution above and beyond other required evidence.  
...
- NIST 800-63A contains additional restrictions on the use of KBV for identity verification at IDIAL2 in section 5.3.2 Knowledge-Based Verification Requirements.
