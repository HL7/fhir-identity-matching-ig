...
This section of the guide extends the existing HL7 FHIR patient [$match](https://www.hl7.org/fhir/patient-operation-match.html) for cross-organizational use by authorized, trusted parties. The IG profiles the operation [$IDI-match] to account for these additional constraints. Conformant systems **SHALL** support the Patient $match operation (or equivalent, if non-FHIR systems are deployed in addition or instead), as further constratined by the $IDI-match profile.
...
This guidance also applies to other matching workflows and to non-FHIR transactions. This guidance does not intend to prohibit the use of matching methods that produce comparable or higher matching rates. Realizing that a better-formed match request produces the most reliable results, this implementation guide (IG) also includes a [Guidance on Identity Assurance] section as a companion resource to this best practice patient matching, which **SHALL** also apply to person matching that may need to occur for non-patient users, though in some sections we also refer to the capability more narrowly as Patient Match. 

> **NOTE:** As security is generally out of scope for this guide, the conditions required to share personally identifiable information (PII) or to authorize an organization's or an individual’s, including the patient’s own, access to the PII or PHI results of a match request are not specified completely in this guide, nor should they be inferred.  However, patient-initiated workflows (for example, "patient request" purpose of use) **SHALL** always include explicit end-user authorization.
>
> Examples:
>
> -Patient or their authorized representative authorizes access, as in SMART App Launch, such that clicking a button to authorize the transaction is the mechanism for capturing and carrying consent in an OAuth authorization code flow transaction, 
> 
> -Tiered OAuth Use Case - similar to the nominal SMART App Launch with user click to authorize within the authorization code flow transaction as described above, except that OpenID Connect user profile data includes demographics verified at sufficiently strong (IDIAL1.8 or higher) identity assurance and the user was authenticated at AAL2 or higher authentication assurance by the trusted identity service, and demographics (optimally, limited demographics and a Digital Identifier) from the trusted identity service can be used to match on the user in real time and make an access decision,
> 
> -B2B with Individual User Use Case - demographics (optimally, limited demographics and a Digital Identifier) in the FHIR Person resource (or equivalent demographics shared in the request, if a different data format is used to share demographics or the PHI transaction is non-FHIR) are consistent with an IDIAL1.8 or higher identity verification event, the user is authenticated at AAL2 or higher authentication assurance, and requester captures and conveys the user's explicit consent each time they invoke Patient Request in an auditable manner supporting the legal basis for the request under HIPAA,
>
> -In all cases, meeting requirements as in the [Consumer Match](patient-matching.html#consumer-match) section below.
...
- in another query, match or search request, or health data push in which a match will be needed by the requester or recipient in order to act on the information exchanged (and note that Consumer Match requirements apply, if PHI or PII will be shared with the patient subsequently--for example if a data push triggers outreach to the patient),
...
Individual Access (or if protected health information -- PHI -- or PII will be shared with someone other than a HIPAA Covered Entity in a Treatment, Payment, or Operations workflow--for example, a Patient Request) requires a minimum identity assurance of IDIAL1.8 or higher for the user and (if different) the patient whose health data is sought, as stipulated in the [Consumer Match](patient-matching.html#consumer-match) section and related [Use Cases](use-cases.html), since responders to any queries involving the release of PHI or PII other than to a HIPAA Covered Entity in a Treatment, Payment, or Operations workflow **SHALL** authenticate the requester and obtain any necessary consent from the patient and/or authorized representative before authorizing PHI or PII to be returned. 

Security best practices, including transaction authorization, are generally out of scope for this IG; however implementers also **SHALL NOT** allow patients to request a match directly except as stipulated in the [](patient-matching.html#consumer-match) section. A trusted system may request a match on a patient’s behalf and use it to inform the patient, especially to:  
...
The term "match" used in this Implementation Guide always involves matching demographics (optimally, limited demographics and a Digital Identifier) bound to a unique person; usually that person is the patient a transaction is about (the person whose data is being requested). In any workflow where health data will be shared other than to a HIPAA Covered Entity, the guidance in this section applies and involves matching on demographics (optimally, limited demographics and a Digital Identifier) for the requester who is the patient, their authorized representative, or another third party--who could be a consumer or even a malicious individual or system, absent strong identity and authentication assurance along with auditable data logs to prove that the appropriate individual was authenticated and authorized each instance of data sharing. In all of these circumstances, the match confidence must meet a higher minimum bar than when PII or PHI will be released to a HIPAA Covered Entity, and the additional requirements in this section **SHALL** apply. 

The quality of the match, along with the user's consent, the verified demographics stored by responder for the patient and any authorized representative(s) of the patient, and identity and authentication assurance levels of the user, inform the responder's decision about whether to allow access to data subsequent to a Consumer Match, and that decision is based on verified attributes and other information and assertions provided by the requester that, for example when part of an [OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html) transaction, **SHALL** also be evaluated by the responder to confirm that the requester is trusted to have authenticated the individual corresponding to those demographics as well as the other details within the transaction, if the user was not authenticated by the responder directly. 

When the patient whose data is being requested is different from the user who is the requester, a high-confidence match on the patient's demographics and/or identifier is required in addition to user-level matching, to confirm the user is an authorized representative, since a request from any other individual **SHALL NOT** result in access to PII or PHI. This second Consumer Match **SHALL** meet the same recommended minimum bar as the match performed on the user. A B2B transaction of this type would authenticate using the B2B with User Authorization Extension Object described in this section and according to the [HL7 UDAP FAST Security IG](https://hl7.org/fhir/us/udap-security), and would then include information about the patient in a match request that is also conformant with this IG. A B2C transaction of this type would begin with user authentication that again includes information about the individual user as in the ID token example in [Requirements for Digital Identifiers for Individuals](digital-identity.html#requirements-for-digital-identifiers-for-individuals); after successful authentication the user is then presented with patient selection options as per the usual SMART App Launch workflow. Refer to the [Artifacts](artifacts.html) section for examples of a patient and authorized representative relationship, to illustrate an identity management practice that facilitates discovery of authorized representative status and successful Consumer Match by a responder.

Whether attempting to match just on the patient or on both the patient and the authorized representative, a match input weight of at least 10, including at a minimum IDI Patient L1 invariant data and IDIAL1.8/AAL2 identity and authentication assurance **SHALL** be included in the match request. That is to say, for a Consumer Match, both the variety of demographics and the identity assurance at which the demographics were verified **SHALL** meet these requriements, at a minimum, or, if demographics are included in a different data format or in other type of transaction (i.e., not a FHIR transaction), equivalent demographics **SHALL** be shared in the match request. 

In workflows without a user-facing component, FHIR client apps invoking client credentials grant **SHALL** use the B2B with User Authorization Extension Object to provide additional information regarding the context under which the client is requesting authorization.The client app constructs a JSON object containing the following keys and values, and includes this object in the extensions object of the Authentication JSON Web Token (JWT), as per [UDAP Security 5.2.1.1](https://hl7.org/fhir/us/udap-security/b2b.html#b2b-authorization-extension-object), as the value associated with the key name hl7-b2b-user. The same technical requirements for use of hl7-b2b apply in the use of hl7-b2b-user. Equivalent assertions and explicit consent **SHALL** also be conveyed in match requests that are non-FHIR transactions.
...
It is a best practice to include all known (required + optional) patient matching attributes in a match request (e.g., USCDI Patient Demographics); the table below indicates examples of n-tuple or n-combination attributes under the category of combinatorics and levels of verification for consideration in different use cases. These combinations of attributes are commonly used to implement n-tuple record linkage. This record linkage pattern allows a newly acquired patient record to be readily mapped to an existing patient record. This de facto standard pattern relies heavily on diverse combinations of PII attributes. In common practice, when working with diverse healthcare data sources no single field, and no single combination (n-tuple) of PII fields is used. You must have combinations that exclude surname because of marriage and divorce.  You must have combinations that exclude first name because both nicknames and shortened first names are commonly used; they will not match the formal name. Also, various combinations of email and/or mobile phone are both highly selective and reliable. MRN can also be selective. Hence, email, phone, MRN readily support excluding last name and/or including similar nicknames to provide still more possible n-tuple combination rules. 
...
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| First Name and Last Name, DOB, and unique Enterprise Identifier | [IDIAL1.5](guidance-on-identity-assurance.html#best-practices-for-identity-verification); onlyCertainMatches and count=1  required for patient care delivery, coverage determination, or  billing/operations | N/A; see below instead                              |
...
| Digital Identifier (HL7PersonIdentifier) | Patient.identifier.system == 'http://hl7.org/fhir/us/identity-matching/ns/HL7PersonIdentifier' |
...
| SSN (last 4) | Patient.identifier.type.coding.system == 'http://hl7.org/fhir/us/identity-matching/CodeSystem/Identity-Identifier-cs' and Patient.identifier.type.coding.code == 'SSN4' OR the last 4 digits of an SSN |
