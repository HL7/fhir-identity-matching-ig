...
Individual Access (or if protected health information--PHI--or PII will be shared with someone other than a HIPAA Covered Entity in a Treatment, Payment, or Operations workflow--for example, a Patient Request) requires a minimum identity assurance of IDIAL1.8 or higher for the user and (if different) the patient whose health data is sought, as stipulated in the [Consumer Match](patient-matching.html#consumer-match) section and related [Use Cases](use-cases.html), since responders to such queries **SHALL** authenticate the individual before returning PHI or PII. 
