...
In an even more specific workflow, the patient whose data is being requested is different from the user who is the requestor; in that case a high-confidence match on the patient's demographics is required in addition to user-level matching on the authorized representative as described in the previous paragraph. This second Consumer Match **SHALL** meet the same recommended minimum bar as the match performed on the user's demographics--match input weight of at least 10, including L1 invariant data (or equivalent), and IAL1.8/AAL2 identity and authentication assurance. 
